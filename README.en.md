...
- 🔁 [Self Improvement Loop](docs/self_improvement.md) with plugin gap detection
- 🧬 [AlphaEvolve strategy evolution](docs/evolve_population.md)
...
3. [Install](#installation) Auto-GPT
...
Auto-GPT requires **Python 3.10 or later**.
...
Edit `.env` and set `OPENAI_API_KEY` and any other required values.
Remove the leading `#` to enable optional settings.
...
The first run will create `auto_gpt_workspace` in your working directory.
See [docs/setup] for more runtime options.

### Command line options

Use the `agpt` command-line tool to launch Auto-GPT or run AlphaEvolve workflows.
`agpt` without subcommands starts Auto-GPT, while `agpt alphaevolve` runs [AlphaEvolve](docs/evolve_population.md).

Common flags include:

- `--working-directory PATH` – location for configuration and data files (defaults to the project root).
...
- `--gpt4only` – use GPT‑4 for all responses.
...
Plugins extend Auto-GPT with extra capabilities. Configuration lives in `plugins_config.yaml` in your working directory.
