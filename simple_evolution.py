...
def evolve_generation(
    gen: int, count: int, retain: int, tasks: list[str]
) -> list[Path]:
    ...
    # Generate mutations
    while len(list(next_dir.glob("*.yaml"))) < count:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Simple offline strategy evolution")
    parser.add_argument(
        "-g", "--generations", type=int, default=3, help="Number of generations"
    )
    parser.add_argument(
        "-p", "--population", type=int, default=10, help="Strategies per generation"
    )
    parser.add_argument(
        "-r", "--retain", type=int, default=3, help="Number of top strategies to keep"
    )
