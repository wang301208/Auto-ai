...
OpenEvolve can also be run from the command line via the unified CLI:

```bash
agpt alphaevolve path/to/initial_program.py path/to/evaluator.py --config path/to/config.yaml --iterations 1000
```
...
agpt alphaevolve examples/function_minimization/initial_program.py \
