    ...
    try:
        ...
        if not existing_items or any(
            ...
            for item in existing_items:
                try:
                    agent.memory.discard(item)
                    if hasattr(agent.memory, "memories") and item in agent.memory.memories:
                        agent.memory.memories.remove(item)
                except Exception:
                    pass
            agent.memory.add(file_memory)
...
def ingest_file(
    ...
    """Ingest a file by reading its content and storing it in memory.
    ...
    try:
        ...
        if ext in code_exts:
            file_memory = MemoryItem.from_code_file(content, filename, config)
        else:
