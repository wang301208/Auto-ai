...
try:
    ...
    openapi_python_client = None  # type: ignore
import requests
...
try:
    ...
    warnings.warn(
        "Optional dependency 'auto-gpt-plugin-template' is not installed."
        " Install it with 'pip install auto-gpt-plugin-template' to enable"
        " plugin support."
    )

    class AutoGPTPluginTemplate:  # type: ignore
...
plugin_registry: dict[str, AutoGPTPluginTemplate] = {}
...
def _write_plugin_registry(
    plugins: list[AutoGPTPluginTemplate], config: "Config"
) -> None:
    ...
    for name, plugin_cfg in config.plugins_config.plugins.items():
...
def load_plugin_commands(command_registry: "CommandRegistry") -> None:
    ...
    for entry in data:
        if entry.get("status") != "enabled":
            continue
        name = entry.get("name")
        description = entry.get("description", "Plugin command")
        cmd_name = name.lower().replace(" ", "_")

        def _placeholder(plugin_name: str = name):
            return f"Plugin {plugin_name} command executed"
                ...
                for key in dir(zipped_module):
                    ...
                    if (
                        ...
                        if plugin_configured and plugin_enabled:
                            ...
                            instance = a_module()
                            loaded_plugins.append(instance)
                            plugin_registry[str(plugin)] = instance
...
def reload_plugin(path: str) -> AutoGPTPluginTemplate | None:
    ...
    try:
        ...
        plugin_class = getattr(module, plugin.__class__.__name__, None)
        if plugin_class and issubclass(plugin_class, AutoGPTPluginTemplate):
            new_instance = plugin_class()
            plugin_registry[path] = new_instance
            return new_instance
        logger.error(
