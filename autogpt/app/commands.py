"""Unified CLI command groups for AutoGPT.

Consolidates all scattered scripts into a single `agpt` command hierarchy:

    agpt              - Start Auto-GPT assistant (default)
    agpt skill        - Skill library management (replaces skill_cli.py + librarian_cli.py)
    agpt orchestrate  - Orchestrator management (replaces run_orchestrator.py + run_blueprint_orchestrator.py)
    agpt evolve       - Algorithm evolution (replaces auto_evolve.py + simple_evolution.py)
    agpt ingest       - Data ingestion (replaces data_ingestion.py)
    agpt tui          - TUI terminal (replaces launch_tui.py)
    agpt plugin       - Plugin management (replaces generate_plugins.py + install_plugin_deps.py)
    agpt governance   - Governance operations (replaces approve_fix.py)
    agpt alphaevolve  - AlphaEvolve workflows (unchanged)
    agpt doctor       - Health check (replaces test_tui.py + tui doctor)
"""
...
def skill_audit() -> None:
    from autogpt.telemetry.audit import get_audit_log
...
@orchestrate.command("start", help=_("Start the AutoGPT orchestrator."))
...
def orchestrate_start(agents: tuple[str], workdir: str, db_path: str) -> None:
    from autogpt.orchestrator import Orchestrator, AVAILABLE_AGENTS
...
def plugin_init_kb(plugin_repo: str, persist_dir: str) -> None:
    from autogpt.config import Config
    from autogpt.memory.vector.utils import get_memory
...
def doctor() -> None:
    ...
    checks = [
        ...
        ("model_router/", _has_spec("autogpt.llm.model_router")),
        ("unified_task", _has_spec("autogpt.agents.unified_task")),
        ("sandbox/", _has_spec("autogpt.sandbox")),
        ("distributed/", _has_spec("autogpt.distributed")),
        ("streaming", _has_spec("autogpt.llm.model_router.streaming")),
        ("self_think", _has_spec("autogpt.agents.self_think")),
...
def model_list(provider: str | None, tier: str | None, local_only: bool) -> None:
    ...
    from autogpt.llm.model_router.model_spec import ModelCapability
...
def model_providers() -> None:
    from autogpt.llm.model_router import ModelRegistry, OllamaProvider
