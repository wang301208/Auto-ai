...
except Exception:  # pragma: no cover - openai>=1.x
    openai = None
import pytest
...
def vcr_cassette_dir(request):
    ...
    return os.path.join("tests/Auto-AI-test-cassettes", test_name)


def patch_api_base(requestor):
    if openai and hasattr(openai, "api_requestor"):
...
def patched_api_requestor(mocker: MockerFixture):
    if not openai or not hasattr(openai, "api_requestor"):
        yield
        return
