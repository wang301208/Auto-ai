...
def test_read_file_refreshes_memory(
    mock_MemoryItem_from_text, agent: Agent, workspace: Workspace
):
    file_path = workspace.get_path("sample.txt")
    with open(file_path, "w", encoding="utf-8") as f:
        f.write("first")

    file_ops.read_file(file_path, agent=agent)
    memories = [m for m in agent.memory if m.metadata.get("location") == str(file_path)]
    assert len(memories) == 1
    assert memories[0].raw_content == "first"
    ...
    memories = [m for m in agent.memory if m.metadata.get("location") == str(file_path)]
    assert len(memories) == 1
    assert memories[0].raw_content == "second"
...
def test_ingest_file_detects_type(
    agent: Agent, mocker: MockerFixture, mock_embedding: Embedding, workspace: Workspace
):
    txt_path = workspace.get_path("note.txt")
    code_path = workspace.get_path("script.py")
    with open(txt_path, "w", encoding="utf-8") as f:
        f.write("hello")
    with open(code_path, "w", encoding="utf-8") as f:
        f.write("print('hi')")

    def make_item(content: str, source_type: str, path: str):
        return MemoryItem(
            raw_content=content,
            summary=content,
            chunks=[content],
            chunk_summaries=[content],
            e_summary=mock_embedding,
            e_chunks=[mock_embedding],
            metadata={"location": path, "source_type": source_type},
        )

    mock_text = mocker.patch.object(
        file_ops.MemoryItem,
        "from_text_file",
        side_effect=lambda content, path, config: make_item(content, "text_file", path),
    )
    mock_code = mocker.patch.object(
        file_ops.MemoryItem,
        "from_code_file",
        side_effect=lambda content, path, config: make_item(content, "code_file", path),
    )

    file_ops.ingest_file(str(txt_path), agent.memory, agent.config)
    file_ops.ingest_file(str(code_path), agent.memory, agent.config)

    assert mock_text.call_count == 1
    assert mock_code.call_count == 1

    assert any(
        m.metadata.get("source_type") == "text_file" and m.metadata.get("location") == str(txt_path)
        for m in agent.memory
    )
    assert any(
        m.metadata.get("source_type") == "code_file" and m.metadata.get("location") == str(code_path)
        for m in agent.memory
    )
