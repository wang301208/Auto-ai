...
def load_plugin_commands(command_registry: "CommandRegistry") -> None:
    ...
    from autoai.models.command_parameter import CommandParameter
    ...
    for plugin_path in [f.path for f in os.scandir(config.plugins_dir) if f.is_dir()]:
        ...
        for member_name, class_obj in inspect.getmembers(plugin):
            if (
                ...
                and AutoAIPluginTemplate in class_obj.__bases__
            ):
                instance = class_obj()
        ...
        if module_list := inspect_zip_for_modules(str(plugin_path), debug):
            for module_name in module_list:
                ...
                for key in dir(zipped_module):
                    ...
                    if (
                        issubclass(a_module, AutoAIPluginTemplate)
                        and a_module.__name__ != "AutoAIPluginTemplate"
                    ):
...
def reload_plugin(path: str) -> AutoAIPluginTemplate | None:
    ...
    try:
        ...
        if plugin_class and issubclass(plugin_class, AutoAIPluginTemplate):
