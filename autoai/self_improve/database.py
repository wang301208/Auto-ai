...
class DatabaseManager:
    ...
    def log_error(self, exception: str, traceback_str: str) -> None:
        ...
        cur.execute(
            ...
            (datetime.now(timezone.utc).isoformat(), exception, traceback_str),
        )
        self.connection.commit()
    ...
    def log_profile(self, name: str, duration: float) -> None:
        ...
        cur.execute(
            ...
            (datetime.now(timezone.utc).isoformat(), name, duration),
        )
        self.connection.commit()
        if self.message_queue:
            self.message_queue.publish(
    ...
    def log_execution(self, description: str, result: str) -> None:
        ...
        cur.execute(
            ...
            (datetime.now(timezone.utc).isoformat(), description, result),
        )
        self.connection.commit()
        if self.message_queue:
            self.message_queue.publish(
    ...
    def log_profile_detail(
        ...
        cur.execute(
            ...
            (datetime.now(timezone.utc).isoformat(), name, function, ncalls, cumtime),
        )
        self.connection.commit()
        if self.message_queue:
            self.message_queue.publish(
