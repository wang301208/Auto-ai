...
The workspace is a directory containing configuration and working files for an AutoAI
agent.
...
class Workspace:
    ...
    @classmethod
    def make_workspace(cls, workspace_directory: str | Path) -> Path:
        ...
        template_files = {
            ".env": repo_root / ".env.template",
            "prompt_settings.yaml": repo_root / "prompt_settings.yaml",
        }

        for filename, template in template_files.items():
            destination = workspace_directory / filename
        ...
        ai_settings_path = workspace_directory / "ai_settings.yaml"
    ...
    def init_workspace_directory(
        ...
        if override_workspace_path is None:
            workspace_path = config.workdir / "auto_ai_workspace"
