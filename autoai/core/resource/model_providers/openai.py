...
from autoai.llm.providers.openai import _get_async_client, _get_client
...
async def _create_embedding(text: str, *_, **kwargs) -> openai.types.Embedding:
    ...
    credentials = {k: kwargs.pop(k) for k in cred_keys if k in kwargs}
    if "engine" in kwargs:
        kwargs["model"] = kwargs.pop("engine")
    client = _get_async_client(credentials)
...
class _OpenAIRetryHandler:
    ...
    _api_key_error_msg = (
        ...
        "read more here: https://docs.aai.co/setup/#getting-an-api-key"
    )
