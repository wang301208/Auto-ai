...
const EmptyState = React.memo(function EmptyState({ info }: { info?: SessionInfo | null }) {
  ...
  return (
    <Box flexDirection="column" paddingTop={1}>
      <Box flexDirection="column" marginBottom={1}>
        ...
        <Text color={theme.colors.dim}>自主运行时终端</Text>
      </Box>

      <Box borderColor={theme.colors.border} borderStyle="round" flexDirection="row" paddingX={2} paddingY={1}>
        <Box flexDirection="column" marginRight={4} width={14}>
          {asciiArt.map((line, index) => (
            <Text color={index < 5 ? theme.colors.assistant : theme.colors.dim} key={`${index}:${line}`}>
              {line}
            </Text>
          ))}
          <Text />
          <Text color={theme.colors.dim} wrap="truncate-end">
            {model}
          </Text>
          <Text color={theme.colors.dim} wrap="truncate-end">
            {cwd}
        ...
        <Box flexDirection="column" width={62}>
          <Box flexDirection="column">
            <Text bold color={theme.colors.prompt}>
              {glyph.cursor} 自然语言交互
            </Text>
